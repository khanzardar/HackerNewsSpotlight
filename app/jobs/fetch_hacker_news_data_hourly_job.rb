...
class FetchHackerNewsDataHourlyJob < ApplicationJob
  queue_as :default

  after_perform do |job|
    self.class.set(wait: 60.minutes).perform_later(job.arguments.first)
  end
  ...
  def perform(*_args)
    # due to simplicity and small size of fetched data, we're going to wipe db and re-seed
    Comment.destroy_all
    Story.destroy_all
